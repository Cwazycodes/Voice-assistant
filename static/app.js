...
function setupRecognition() {
    ...
    recognition.onresult = function (event) {
        ...
        for (let i = event.resultIndex; i < event.results.length; ++i) {
            if (event.results[i].isFinal) {
                ...
                interimTranscript += event.results[i][0].transcript.toLowerCase().trim();
            }
        }
    ...
    recognition.onerror = function (event) {
        ...
        if (event.error === "not-allowed") {
            document.getElementById("status").innerText = "Microphone blocked. Please allow access.";
        }
    };
...
function speakText(text) {
    ...
    utterance.onend = function () {
        isSpeaking = false; 
        document.getElementById("status").innerText = "Finished speaking. Listening again...";
